  ...
  template: `
    @if (settingsService.showHelp()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" (click)="close()">
        <div class="bg-white dark:bg-weather-stormy rounded-3xl shadow-2xl max-w-md w-full" (click)="$event.stopPropagation()">
          <div class="p-8">
            ...
            <div class="space-y-6">
              ...
              <div class="space-y-3">
                ...
                >
                  ...
                  <span [class]="settingsService.soundEnabled() ? 'text-weather-clear' : 'text-slate-400'">
              ...
              <div class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                    <div class="text-2xl font-black text-weather-sunny">{{ statsService.totalSearches() }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Searches</div>
                  </div>
                  <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                    <div class="text-2xl font-black text-weather-rainy">{{ statsService.totalLocationsSaved() }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Saved</div>
                  </div>
                  <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                    <div class="text-2xl font-black text-weather-sunny">{{ statsService.formatTime() }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Time Spent</div>
                  </div>
                  <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center">
                    <div class="text-2xl font-black text-weather-rainy">{{ statsService.lastSearchDate() ? 'Yes' : 'No' }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Last Search</div>
                  </div>
                </div>
                <button
                  (click)="resetStats()"
