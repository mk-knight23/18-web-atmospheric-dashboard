...
@Component({
  ...
  template: `
    <div
      class="min-h-screen relative flex flex-col items-center"
    ...
    >
      ...
      <!-- Navigation / Search -->
      <nav
        class="w-full max-w-6xl px-6 py-10 flex flex-col md:flex-row justify-between items-center gap-8"
      >
        <div class="flex items-center space-x-3">
          <div
            class="bg-white/20 p-2.5 rounded-2xl backdrop-blur-md border border-white/30 shadow-lg"
          >
        ...
        <div class="relative w-full max-w-md group">
          <lucide-icon
            [name]="'search'"
            class="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500"
            [size]="20"
          ></lucide-icon>
          <input
            type="text"
        ...
        <div class="flex items-center space-x-4">
          <button
            (click)="openSettings()"
            class="p-3 rounded-2xl bg-white/20 backdrop-blur-md border border-white/20 text-white hover:bg-white/30 transition-all shadow-lg"
          >
          ...
          <button
            (click)="weatherService.toggleUnits()"
            class="p-3 rounded-2xl bg-white/20 backdrop-blur-md border border-white/20 text-white font-black text-sm uppercase tracking-widest hover:bg-white/30 transition-all shadow-lg px-6"
          >
      ...
      <main class="w-full max-w-6xl px-6 pb-20 space-y-12">
        @if (weatherService.isLoading()) {
          <div class="flex flex-col items-center justify-center py-40">
            ...
            ></div>
            <p class="mt-4 text-white font-black uppercase tracking-[0.3em] text-[10px]">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
            <!-- Hero Weather -->
            <div
              class="lg:col-span-8 glass-card p-12 flex flex-col md:flex-row justify-between items-center relative overflow-hidden group"
            >
              <div class="space-y-4 z-10 text-center md:text-left">
                <div
                  class="flex items-center justify-center md:justify-start space-x-2 text-blue-500 font-black uppercase tracking-[0.2em] text-xs"
                >
                  <lucide-icon [name]="'map-pin'" [size]="14"></lucide-icon>
                  <span>{{ data.city }}, {{ data.country }}</span>
                </div>
                <h2
                  class="text-8xl md:text-9xl font-display font-black tracking-tighter text-slate-900 dark:text-white"
                >
                  {{ data.temp | number: '1.0-0' }}°
                </h2>
                <div class="space-y-1">
                  <p class="text-2xl font-bold text-slate-700 dark:text-slate-300 capitalize">
                    {{ data.description }}
                  </p>
                  <p class="text-slate-500 dark:text-slate-500 font-medium">
                    Feels like {{ data.feelsLike | number: '1.0-0' }}°
                  </p>
                </div>
              </div>
              ...
              <div class="z-10 mt-10 md:mt-0">
                <img
                  ...
                  class="w-64 h-64 drop-shadow-2xl animate-pulse"
              ...
              <div
                class="absolute top-0 right-0 p-4 opacity-5 pointer-events-none group-hover:rotate-12 transition-transform duration-1000"
              >
          ...
          <div class="max-w-md mx-auto glass-card p-12 text-center space-y-6">
            <div
              class="w-20 h-20 bg-red-50 dark:bg-red-900/30 text-red-500 rounded-3xl flex items-center justify-center mx-auto text-4xl"
            >
              !
            </div>
            <div class="space-y-2">
            ...
            <button
              (click)="onSearch()"
              class="text-blue-500 font-black uppercase tracking-widest text-xs hover:underline"
            >
              Retry Connection
            </button>
          </div>
          ...
          <div class="max-w-2xl mx-auto py-20 text-center space-y-12">
            <div class="space-y-4">
              <h2
                class="text-5xl md:text-7xl font-display font-black tracking-tight text-white drop-shadow-xl animate-bounce"
              >
                Hello.
              </h2>
              <p class="text-white/80 text-xl font-medium italic">
                Search a city to begin scanning the horizon.
              </p>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 opacity-60">
              <div
                class="p-6 bg-white/10 backdrop-blur-md rounded-3xl border border-white/10 text-white"
              >
                NYC
              </div>
              <div
      ...
      <footer
        class="mt-auto py-12 px-6 border-t border-slate-200 dark:border-slate-800 w-full flex flex-col md:flex-row justify-between items-center gap-8 text-slate-400"
      >
  ...
  styles: [
    `
      ...
      .input-search::placeholder {
        @apply text-slate-400 font-medium tracking-tight;
      }
    `,
  ],
})
